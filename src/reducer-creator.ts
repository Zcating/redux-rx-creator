...
export function createReducer<S, A extends Action = Action>(
    ...
    for (const theOn of ons) {
        for (const type of theOn.types) {
            if (map.has(type)) {
                const existingReducer = map.get(type) as ActionReducer<S, A>;
                const newReducer: ActionReducer<S, A> = (state, action) => theOn.reducer(existingReducer(state, action), action);
                map.set(type, newReducer);
            } else {
                map.set(type, theOn.reducer);
            }
        }
    }
